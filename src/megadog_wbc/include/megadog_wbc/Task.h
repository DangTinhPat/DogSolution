#ifndef CONTROLLER_CONTROL_TASK_H
#define CONTROLLER_CONTROL_TASK_H

// Ported near-verbatim from skywoodsz/qm_control's qm_wbc::Task (itself a
// generic hierarchical-QP task type with no robot-specific content) - see
// src/megadog_wbc/HoQp.h for the solver that consumes it and
...
// A single priority level's constraint set: Ax = b (equality), Dx <= f
// (inequality). HoQp stacks these across priority levels via operator+.
class Task
{
public:
    ...
    {
        return {a_.cols() > 0 ? rhs * a_ : a_, b_.cols() > 0 ? rhs * b_ : b_,
                d_.cols() > 0 ? rhs * d_ : d_, f_.cols() > 0 ? rhs * f_ : f_};
    }
...
#endif  // CONTROLLER_CONTROL_TASK_H
